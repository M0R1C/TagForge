...
import sys
print(f"[DEBUG] database.py __file__ = {__file__}")
print(f"[DEBUG] DB_PATH = {os.path.join(os.path.dirname(__file__), 'dataset_editor.db')}")
sys.stdout.flush()
...
class Database:
    ...
    def save_image_rating(self, dataset_path, filename, rating, file_hash=None):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('''
    ...
    def get_image_rating_and_hash(self, dataset_path, filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def get_image_rating(self, dataset_path, filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def get_all_ratings_for_dataset(self, dataset_path):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('SELECT filename, rating FROM image_ratings WHERE dataset_path=?', (dataset_path,))
            return {row[0]: row[1] for row in c.fetchall()}

    def save_image_quality(self, dataset_path, filename, metrics):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('''
    ...
    def get_image_quality(self, dataset_path, filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def delete_image_quality(self, dataset_path, filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('DELETE FROM image_quality WHERE dataset_path=? AND filename=?', (dataset_path, filename))
            conn.commit()

    def update_duplicate_groups(self, dataset_path):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            ...
            for file_hash, files in groups.items():
                if len(files) > 1:
                    c.execute('''
                        ...
                        VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                    ''', (dataset_path, file_hash, json.dumps(files)))
            conn.commit()

    def remove_from_duplicate_groups(self, dataset_path, filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('SELECT file_hash FROM image_quality WHERE dataset_path=? AND filename=?',
                      (dataset_path, filename))
            row = c.fetchone()
            if row and row[0]:
                file_hash = row[0]
                ...
                if row2:
                    ...
                    if filename in files:
                        ...
                        if len(files) <= 1:
                            c.execute('DELETE FROM duplicate_groups WHERE dataset_path=? AND file_hash=?',
                                      (dataset_path, file_hash))
                        else:
                            c.execute(
                                'UPDATE duplicate_groups SET files=?, updated_at=CURRENT_TIMESTAMP WHERE dataset_path=? AND file_hash=?',
                                (json.dumps(files), dataset_path, file_hash))
            conn.commit()

    def rename_file_in_duplicate_groups(self, dataset_path, old_filename, new_filename, file_hash):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('SELECT files FROM duplicate_groups WHERE dataset_path=? AND file_hash=?',
                      (dataset_path, file_hash))
    ...
    def update_similar_pairs(self, dataset_path, threshold=15):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('''
    ...
    def remove_from_similar_pairs(self, dataset_path, filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def rename_file_in_similar_pairs(self, dataset_path, old_filename, new_filename):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def rename_flag(self, old_name, new_name, new_color):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('UPDATE dataset_flags SET name=?, color=? WHERE name=?', (new_name, new_color, old_name))
            conn.commit()

    def save_global_widget_layout(self, layout_json):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('DELETE FROM widget_layouts')
            c.execute('INSERT INTO widget_layouts (layout, updated_at) VALUES (?, CURRENT_TIMESTAMP)', (layout_json,))
            conn.commit()

    def get_global_widget_layout(self):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def save_dataset_vocabulary(self, dataset_path, content_json):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute('''
    ...
    def get_dataset_vocabulary(self, dataset_path):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def rename_folder_prefix_in_db(self, old_prefix, new_prefix):
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
    ...
    def get_version_stats(self, version_path):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for img in images:
                ...
                row = c.fetchone()
                if row:
                    if row[0]: mult32_count += 1
                    if row[1]: mult64_count += 1
                else:
                    ...
                    try:
                        ...
                        mult32 = False
                        mult64 = False
                    if mult32: mult32_count += 1
                    if mult64: mult64_count += 1
        total_tags_count = 0
        all_tags_set = set()
        for img in images:
...
def save_image_rating(dataset_path, filename, rating, file_hash=None):
    return _get_db().save_image_rating(dataset_path, filename, rating, file_hash)

def get_image_rating_and_hash(dataset_path, filename):
    return _get_db().get_image_rating_and_hash(dataset_path, filename)

def get_image_rating(dataset_path, filename):
    return _get_db().get_image_rating(dataset_path, filename)

def get_all_ratings_for_dataset(dataset_path):
    return _get_db().get_all_ratings_for_dataset(dataset_path)

def save_image_quality(dataset_path, filename, metrics):
    return _get_db().save_image_quality(dataset_path, filename, metrics)

def get_image_quality(dataset_path, filename):
    return _get_db().get_image_quality(dataset_path, filename)

def delete_image_quality(dataset_path, filename):
    return _get_db().delete_image_quality(dataset_path, filename)

def update_duplicate_groups(dataset_path):
    return _get_db().update_duplicate_groups(dataset_path)

def remove_from_duplicate_groups(dataset_path, filename):
    return _get_db().remove_from_duplicate_groups(dataset_path, filename)

def rename_file_in_duplicate_groups(dataset_path, old_filename, new_filename, file_hash):
    return _get_db().rename_file_in_duplicate_groups(dataset_path, old_filename, new_filename, file_hash)

def update_similar_pairs(dataset_path, threshold=15):
    return _get_db().update_similar_pairs(dataset_path, threshold)

def remove_from_similar_pairs(dataset_path, filename):
    return _get_db().remove_from_similar_pairs(dataset_path, filename)

def rename_file_in_similar_pairs(dataset_path, old_filename, new_filename):
    return _get_db().rename_file_in_similar_pairs(dataset_path, old_filename, new_filename)

def rename_folder_prefix_in_db(old_prefix, new_prefix):
    return _get_db().rename_folder_prefix_in_db(old_prefix, new_prefix)

def get_all_flags():
    return _get_db().get_all_flags()

def add_flag(name, color):
    return _get_db().add_flag(name, color)

def delete_flag(name):
    return _get_db().delete_flag(name)

def rename_flag(old_name, new_name, new_color):
    return _get_db().rename_flag(old_name, new_name, new_color)

def save_global_widget_layout(layout_json):
    return _get_db().save_global_widget_layout(layout_json)

def get_global_widget_layout():
    return _get_db().get_global_widget_layout()

def save_dataset_vocabulary(dataset_path, content_json):
    return _get_db().save_dataset_vocabulary(dataset_path, content_json)

def get_dataset_vocabulary(dataset_path):
    return _get_db().get_dataset_vocabulary(dataset_path)

def get_all_translations(lang):
    return _get_db().get_all_translations(lang)

def clear_all_analysis_data():
    return _get_db().clear_all_analysis_data()

def get_version_stats(version_path):
    return _get_db().get_version_stats(version_path)
